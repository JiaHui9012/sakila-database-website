...
<html>
	...
	<body>
		...
		<?php require_once 'categoryprocess.php';	?>
		...
		<form action="categoryprocess.php" method="POST">
			...
			<div class="in-group">
				...
				<input type="number" name="category_id" class="in-input" value="<?php echo $category_id; ?>" placeholder="Enter category id" required>
			</div>
			<div class="in-group">
				<label>name</label><br/>
        ...
        <form>
			...
			?>
				<table align="left" border="1px" style="width: 70%; line-height: 40px;">
					<tr>
						<?php if (!empty($_POST['select'])&&in_array("category_id",$_POST['select'])){ ?>
							<th>category_id</th>
						<?php }
						else{ ?>
					...
					<?php while ($rows = $result -> fetch_assoc()){ ?>
						<tr align="center">
							...
							<?php } ?>
							<td><button type="submit" name="update"><a href="category.php?update=<?php echo $rows['category_id']; ?>">Update</a></button>
								<button type="submit" name="delete"><a href="categoryprocess.php?delete=<?php echo $rows['category_id']; ?>" onclick="return confirm('Are You Sure?');">Delete</a></button>
							</td>
						</tr>
					<?php } ?>
				</table>	
