...
<html>
	...
	<body>
		...
		<?php require_once 'actorprocess.php';	?>
		...
		<div class="insert">
		<form action="actorprocess.php" method="POST">
			<h3>Enter Information</h3>
			<input type="hidden" name="actor_id" value="<?php echo $actor_id; ?>">
			<div class="in-group">
				<label>actor_id</label><br/>
				<input type="number" name="actor_id" class="in-input" value="<?php echo $actor_id; ?>" placeholder="Enter actor id" required>
			</div>
			<div class="in-group">
				<label>first_name</label><br/>
				<input type="text" name="first_name" class="in-input" value="<?php echo $first_name; ?>" placeholder="Enter first name">
			</div>
			<div class="in-group">
				<label>last_name</label><br/>
				<input type="text" name="last_name" class="in-input" value="<?php echo $last_name; ?>" placeholder="Enter last name">
			</div>
			<div>
			<?php
			if ($update == true):
				...
				<table align="left" border="1px" style="width: 70%; line-height: 40px;">
					...
					<?php while ($rows = $result -> fetch_assoc()){ ?>
						<tr align="center">
							...
							<?php } ?>
							<td><button type="submit" name="update"><a href="actor.php?update=<?php echo $rows['actor_id']; ?>">Update</a></button>
								<button type="submit" name="delete"><a href="actorprocess.php?delete=<?php echo $rows['actor_id']; ?>" onclick="return confirm('Are You Sure?');">Delete</a></button>
							</td>
						</tr>
					<?php } ?>
				</table>	
