...
<html>
	...
	<body>
		...
		<?php require_once 'cityprocess.php';	?>
		...
		<form action="" method="POST">
			...
			<label>city_id</label>
			<input type="checkbox" name="select[]" value="city" <?php echo (isset($_POST['select-button']))? ((!empty($_POST['select'])&&in_array("city",$_POST['select']))? "checked" : "") : "checked"; ?>>
			<label>city</label>
		...
		<div class="insert">
		<form action="cityprocess.php" method="POST">
			<h3>Enter Information</h3>
			<input type="hidden" name="city_id" value="<?php echo $city_id; ?>">
			<div class="in-group">
				<label>city_id</label><br/>
				<input type="number" name="city_id" class="in-input" value="<?php echo $city_id; ?>" placeholder="Enter city id" required>
			</div>
			<div class="in-group">
				<label>city</label><br/>
				<input type="text" name="city" class="in-input" value="<?php echo $city; ?>" placeholder="Enter city" required>
			</div>
			<div class="in-group">
				<label>country_id</label><br/>
				<input type="number" name="country_id" class="in-input" value="<?php echo $country_id; ?>" placeholder="Enter country_id" required>
			</div>
			<div>
			<?php
			if ($update == true):
        ...
        <form>
			...
			?>
				<table align="left" border="1px" style="width: 70%; line-height: 40px;">
					<tr>
						<?php if (!empty($_POST['select'])&&in_array("city_id",$_POST['select'])){ ?>
							<th>city_id</th>
						<?php }
						else{ ?>
							<th style="display:none;">city_id</th>
						<?php }
						if (!empty($_POST['select'])&&in_array("city",$_POST['select'])){ ?>
							<th>city</th>
						<?php }
						else{ ?>
							<th style="display:none;">city</th>
						<?php }
						if (!empty($_POST['select'])&&in_array("country_id",$_POST['select'])){ ?>
							<th>country_id</th>
						<?php }
						else{ ?>
					...
					<?php while ($rows = $result -> fetch_assoc()){ ?>
						<tr align="center">
							...
							<?php }
								...
								<td style="display:none;"><?php echo $rows['city']; ?></td>
							<?php }
							if (!empty($_POST['select'])&&in_array("country_id",$_POST['select'])){ ?>
							...
							<?php } ?>
							<td><button type="submit" name="update"><a href="city.php?update=<?php echo $rows['city_id']; ?>">Update</a></button>
								<button type="submit" name="delete"><a href="cityprocess.php?delete=<?php echo $rows['city_id']; ?>" onclick="return confirm('Are You Sure?');">Delete</a></button>
							</td>
						</tr>
					<?php } ?>
				</table>	
