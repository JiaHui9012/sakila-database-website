	...
	<body>
		...
		<form action="inventoryprocess.php" method="POST">
            ...
            <div class="in-group">
				...
				<input type="number" name="store_id" class="store_id" value="<?php echo $store_id; ?>" placeholder="Enter store_id" required>
			</div>
            <div>
			<?php
			if ($update == true):
        ...
        <form>
			...
			$result = $mysqli->query("SELECT * FROM inventory") or die($mysqli->error);
			
			if (isset($_POST['select-button'])){
			?>
				<table align="left" border="1px" style="width: 80%; line-height: 40px;">
					<tr>
						<?php if (!empty($_POST['select'])&&in_array("inventory_id",$_POST['select'])){ ?>
							<th>inventory_id</th>
						<?php }
						else{ ?>
					...
					<?php while ($rows = $result -> fetch_assoc()){ ?>
						<tr align="center">
							...
							<?php } ?>
							<td><button type="submit" name="update"><a href="inventory.php?update=<?php echo $rows['inventory_id']; ?>">Update</a></button>
								<button type="submit" name="delete"><a href="inventoryprocess.php?delete=<?php echo $rows['inventory_id']; ?>" onclick="return confirm('Are You Sure?');">Delete</a></button>
							</td>
						</tr>
					<?php } ?>
				</table>	
