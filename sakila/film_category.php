...
<html>
	...
	<body>
		...
		<?php require_once 'film_categoryprocess.php';	?>
		...
		<form action="" method="POST">
			...
			<label>film_id</label>
			<input type="checkbox" name="select[]" value="category_id" <?php echo (isset($_POST['select-button']))? ((!empty($_POST['select'])&&in_array("category_id",$_POST['select']))? "checked" : "") : "checked"; ?>>
		...
		<div class="insert">
		<form action="film_categoryprocess.php" method="POST">
			...
			<div class="in-group">
				...
				<input type="number" name="category_id" class="in-input" value="<?php echo $category_id; ?>" placeholder="Enter category_id" required>
			</div>
			<div>
			<?php
			if ($update == true):
        ...
        <form>
			...
			$result = $mysqli->query("SELECT * FROM film_category") or die($mysqli->error);
			...
			?>
				<table align="left" border="1px" style="width: 70%; line-height: 40px;">
					<tr>
						...
						<?php }
						if (!empty($_POST['select'])&&in_array("category_id",$_POST['select'])){ ?>
							<th>category_id</th>
						<?php }
						else{ ?>
					...
					<?php while ($rows = $result -> fetch_assoc()){ ?>
						<tr align="center">
							...
							<?php }
								...
								<td style="display:none;"><?php echo $rows['category_id']; ?></td>
							<?php }
							if (!empty($_POST['select'])&&in_array("last_update",$_POST['select'])){ ?>
							...
							<?php } ?>
							<td><button type="submit" name="update"><a href="film_category.php?update=<?php echo $rows['film_id']; ?>">Update</a></button>
								<button type="submit" name="delete"><a href="film_categoryprocess.php?delete=<?php echo $rows['film_id']; ?>" onclick="return confirm('Are You Sure?');">Delete</a></button>
							</td>
						</tr>
					<?php } ?>
				</table>	
